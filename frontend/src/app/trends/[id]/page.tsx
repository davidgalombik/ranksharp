...
/** Full-bleed image strip — up to 5 images side by side, masonry-like heights */
function HeroMosaic({ examples }: { examples: TrendExample[] }) {
  const withImg = examples.filter((e) => e.primary_image_url).slice(0, 5);

  if (withImg.length === 0) {
    return (
      <div className="h-64 bg-stone-100 flex items-center justify-center text-stone-300 text-6xl rounded-xl">
        ⌂
      </div>
    );
  }
  ...
  // 3-5 images: large left + right column
...
/** Portrait product card for the examples grid */
function ProductCard({ example }: { example: TrendExample }) {
  return (
    <a
      href={example.url}
      target="_blank"
    ...
    >
      ...
      <div className="relative h-48 bg-stone-100 flex-shrink-0 overflow-hidden">
        ...
        <span
          ...
          title={example.retailer_country}
        >
          {COUNTRY_FLAGS[example.retailer_country] ?? example.retailer_country}
        </span>
        {example.is_hero && (
          <span className="absolute top-2 right-2 text-xs bg-amber-400 text-white px-1.5 py-0.5 rounded font-medium">
            Hero
          </span>
        )}
      </div>
      ...
      <div className="p-3 flex flex-col gap-1 flex-1">
        ...
        {example.price != null && (
          <p className="text-sm font-semibold text-stone-700">
            {formatPrice(example.price, example.currency)}
          </p>
        )}
...
export default async function TrendDetailPage({ params }: { params: { id: string } }) {
  let trend: Trend;
  try {
    trend = await api.trends.get(parseInt(params.id));
  } catch {
    notFound();
  }

  const mosaicExamples = trend.examples.slice(0, 5);
  const allExamples = trend.examples;

  const attributeSections = [
    { label: "Colours",   items: trend.dominant_colours },
    { label: "Materials", items: trend.dominant_materials },
    { label: "Patterns",  items: trend.dominant_patterns },
    { label: "Styles",    items: trend.dominant_styles },
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <nav className="text-sm text-stone-500">
        <Link href="/trends" className="hover:text-stone-900">Trends</Link>
        {" / "}
        <span className="text-stone-900">{trend.name}</span>
      </nav>

      {/* Hero mosaic */}
      <HeroMosaic examples={mosaicExamples} />
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            ...
            {trend.momentum_pct != null &&
              ` (${trend.momentum_pct > 0 ? "+" : ""}${trend.momentum_pct}% vs last week)`}
          </span>
            ...
            <div key={label} className="bg-white rounded-xl border border-stone-200 p-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {items.map((item) => (
                  <span
                    key={item}
                    className="px-2 py-0.5 bg-stone-100 rounded-full text-sm text-stone-700"
                  >
                    {item}
                  </span>
                ))}
      ...
      <section className="bg-white rounded-xl border border-stone-200 p-6">
        ...
        <div className="flex flex-wrap gap-2">
          {trend.retailer_names.map((name) => (
            <span
              key={name}
              className="px-3 py-1 bg-stone-50 border border-stone-200 rounded-full text-sm text-stone-700"
            >
              {name}
            </span>
          ))}
