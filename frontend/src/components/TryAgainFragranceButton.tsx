 ...
 * Middle → Mass, ~15 min).
 */
export default function TryAgainFragranceButton({ segment }: { segment?: string }) {
  ...
  async function handleClick(choice?: SegmentChoice) {
    ...
    let taskId: string;
    try {
      const res = await api.fragranceTrends.regenerate(effective);
      taskId = res.task_id;
    } catch {
      setPhase("error");
    ...
    pollRef.current = setInterval(async () => {
      try {
        const status = await api.fragranceTrends.taskStatus(taskId);
        setPct(status.pct);
        setStep(status.step);
