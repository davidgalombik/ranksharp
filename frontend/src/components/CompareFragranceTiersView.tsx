...
const SIGNAL_LABELS: Record<Signal, { label: string; blurb: string; badge: string }> = {
  "luxury-only":   { label: "🌱 Emerging in Luxury",  blurb: "Early signal — may diffuse to Middle / Mass in coming runs",
...
/**
 * Fragrance diffusion view — same-name fragrance trends compared across
 * Luxury / Middle / Mass.
 *
 * Reads /api/fragrance-trends/compare. Unlike Product Trends this takes
 * each tier's latest run independently (fragrance runs are not
 * week-aligned across tiers), then merges by (name, category).
 */
export default function CompareFragranceTiersView() {
    ...
    <div className="space-y-4">
      <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
        ...
        <p className="text-xs text-purple-700 mt-1 leading-relaxed">
          Same-name fragrance trends compared across Luxury, Middle, and
          Mass. Each tier's most recent run is used independently (fragrance
          runs are not aligned by week across tiers). A trend in Luxury only
          is often a leading indicator — Mass adoption may follow.
        </p>
        {week && (
          <p className="text-xs text-purple-500 mt-1">Latest run: {week}</p>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-stone-50 border border-stone-200 rounded-xl p-12 text-center">
          ...
          <p className="text-stone-600 font-medium">
            {rows.length === 0
              ? "No segmented fragrance trends yet"
              : "No trends match this signal filter"}
          </p>
          {rows.length === 0 && (
            <p className="text-stone-400 text-sm mt-1">
              Run fragrance analysis for at least one tier — Luxury, Middle,
              or Mass — to populate this view.
            </p>
          )}
        </div>
      ) : (
...
function TierCol({ cell, bg }: { cell: TierCell; bg?: string }) {
  ...
  return (
    <td className={clsx("px-4 py-2 text-right tabular-nums", bg)}>
      <Link href={`/fragrance-trends/${cell.trend_id}`}
