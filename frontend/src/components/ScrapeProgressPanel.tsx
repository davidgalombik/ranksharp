...
function OverallBar({ data }: { data: OverallProgress }) {
  const { retailers_done: done, retailers_running: running, retailers_failed: failed, total_retailers: total } = data;
  return (
    <div className="bg-white rounded-xl border border-stone-200 p-6 space-y-4">
      ...
      <div className="h-4 w-full rounded-full bg-stone-100 overflow-hidden flex">
        <div className="h-full bg-emerald-500 transition-all duration-700" style={{ width: `${(done / total) * 100}%` }} />
        <div className="h-full bg-blue-400 animate-pulse transition-all duration-700" style={{ width: `${(running / total) * 100}%` }} />
        <div className="h-full bg-rose-400 transition-all duration-700" style={{ width: `${(failed / total) * 100}%` }} />
      </div>
      ...
      <div className="flex gap-6 text-sm text-stone-600 pt-1 border-t border-stone-100">
        <span>
          ...
          </strong>{" "}total products in DB
        </span>
        {data.total_products_new > 0 && (
          <span><strong className="text-stone-900">+{data.total_products_new.toLocaleString()}</strong> new this run</span>
        )}
        {running > 0 && (
          <span className="text-blue-600 font-medium animate-pulse">
            ● {running} scrape{running > 1 ? "s" : ""} in progress
          </span>
        )}
      </div>
    </div>
...
function RetailerRow({
  ...
  return (
    <div className={clsx("flex flex-col sm:flex-row sm:items-center gap-3 px-4 py-3 rounded-xl border transition-colors", cfg.row)}>
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className={clsx("px-2 py-0.5 rounded-full text-xs font-medium", cfg.badge)}>
              {cfg.label}
              {job.status === "running" && job.started_at && <> · {elapsed(job.started_at)}</>}
              {job.status === "success" && job.duration_seconds != null && <> · {fmt(job.duration_seconds)}</>}
            </span>
          </div>
          {(job.status === "running" || job.status === "success" || job.status === "never_run") && (
            <p className="text-xs text-stone-500 mt-0.5">
              {job.status === "running" && job.total_products_in_db > 0 && (
                <span className="text-blue-700 font-medium">{job.total_products_in_db.toLocaleString()} saved so far… </span>
              )}
              {job.status === "success" && job.products_found > 0 && (
                <>
                  {job.products_found.toLocaleString()} scraped
                  {job.products_new > 0 && <> · <span className="text-emerald-700">+{job.products_new.toLocaleString()} new</span></>}
                  {job.products_updated > 0 && <> · {job.products_updated.toLocaleString()} updated</>}
                  {" · "}
                </>
              )}
              {job.total_products_in_db > 0 && <span>{job.total_products_in_db.toLocaleString()} total in DB</span>}
            </p>
          )}
          {job.status === "running" && (
            <div className="mt-1.5 h-1.5 w-48 rounded-full bg-stone-200 overflow-hidden">
              <div className="h-full bg-blue-400 rounded-full animate-[progress_2s_ease-in-out_infinite]" style={{ width: "60%" }} />
            </div>
          )}
