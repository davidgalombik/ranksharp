...
async def init_db():
    ...
    async with async_engine.begin() as conn:
        ...
        migrations = [
            ...
            ("ALTER TABLE products ADD COLUMN IF NOT EXISTS subcategory VARCHAR(500)",
             _col("products", "subcategory")),
            ("ALTER TABLE products ADD COLUMN IF NOT EXISTS product_segment VARCHAR(500)",
             _col("products", "product_segment")),
            ...
            # Aldi product clusters (2026-08-06). AldiProductIdea rows
            # generated by the NEW clustering flow store per-cluster
            # keywords used for the live "View all N products" query,
            # and `kind='cluster'` so the frontend renders them with the
            # new sub-theme card UI. Legacy rows keep kind NULL and the
            # old IdeaCard UI so historical sessions still work.
            ("ALTER TABLE aldi_product_ideas ADD COLUMN IF NOT EXISTS filter_keywords JSONB DEFAULT '[]'",
